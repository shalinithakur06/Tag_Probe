...

{
  ...
  tree_->Branch("ele_dEtaInSeed", &ele_dEtaInSeed_);  ///added by me from 163-182
  tree_->Branch("ele_iEminusiP", &ele_iEminusiP_);
  tree_->Branch("ele_nInnerHits", &ele_nInnerHits_);
  tree_->Branch("ele_nInnerLostHits", &ele_nInnerLostHits_);
  tree_->Branch("ele_isPassConVeto", &ele_isPassConVeto_);
  tree_->Branch("ele_isEcalDriven", &ele_isEcalDriven_);
  tree_->Branch("ele_energy5x5", &ele_energy5x5_);
  tree_->Branch("ele_energy2x5", &ele_energy2x5_);
  tree_->Branch("ele_energy1x5", &ele_energy1x5_);
  tree_->Branch("ele_eleRho", &ele_eleRho_);
  tree_->Branch("ele_eleTrkPt", &ele_eleTrkPt_);
  tree_->Branch("ele_GsfEleEmHadD1IsoRhoCut", &ele_GsfEleEmHadD1IsoRhoCut_);
  tree_->Branch("ele_relCombPFIsoEA", &ele_relCombPFIsoEA_);
...
//  tree_->Branch("isTrue"    , &isTrue_);
 ...
 // Electron Trigger branch
  ...
  tree_->Branch("passFilterEle27" ,  &passFilterEle27);      ////// added by me
...
Ntupler::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  TString ele_filters[6] = {"hltEle35noerWPTightGsfTrackIsoFilter","hltEle23Ele12CaloIdLTrackIdLIsoVLTrackIsoLeg1Filter","hltEle23Ele12CaloIdLTrackIdLIsoVLTrackIsoLeg2Filter","hltMu12TrkIsoVVLEle23CaloIdLTrackIdLIsoVLElectronlegTrackIsoFilter","hltMu23TrkIsoVVLEle12CaloIdLTrackIdLIsoVLElectronlegTrackIsoFilter","hltEle27WPTightGsfTrackIsoFilter"};
...
//           cout<<"filterIndex : "<<filterIndex<<"   , filterName :  "<<(*triggerSummary).filterLabel(filterIndex)<<"  , filterTag : "<<filterTag<<endl;
  ...
  //        obj.unpackPathNames(names);
    ...
    if(doEle_)
    {
      ...
      ele_dEtaInSeed_.clear();  ////added by me from 562-580
      ele_iEminusiP_.clear(); 
      ele_nInnerHits_.clear(); 
      ele_nInnerLostHits_.clear();
      ele_isPassConVeto_.clear();
      ele_isEcalDriven_.clear();
      ele_energy5x5_.clear();
      ele_energy2x5_.clear();
      ele_energy1x5_.clear();
      ele_eleRho_.clear();
      ele_eleTrkPt_.clear();
      ele_GsfEleEmHadD1IsoRhoCut_.clear();
      ele_relCombPFIsoEA_.clear();
      ele_PileupIso_.clear();
      ele_isEE_.clear();
      ele_isEB_.clear(); 
//      ele_isTrue_.clear();
      ...
      bool filterEle27 = false;  ///added by me
...
//      
        ...
        if(ele_filters[5].Contains(filter) && foundTheLeg)  filterEle27 = true;  ///added by me
      }
      
      passFilterEle27  .push_back(filterEle27);  ///added by me
      ...
      ele_dEtaInSeed_.push_back( el->superCluster().isNonnull() && el->superCluster()->seed().isNonnull() ?el->deltaEtaSuperClusterTrackAtVtx() - el->superCluster()->eta() + el->superCluster()->seed()->eta() : std::numeric_limits<float>::max() ); ////added by me from 654-669
      const float ecal_energy_inverse = 1.0/el->ecalEnergy();
      const float eSCoverP = el->eSuperClusterOverP();
      ele_iEminusiP_.push_back( fabs(1.0 - eSCoverP)*ecal_energy_inverse );
      ///ele_nInnerHits_.push_back( el->gsfTrack()->hitPattern().numberOfAllHits(reco::HitPattern::MISSING_INNER_HITS) );
      ele_nInnerLostHits_.push_back ( el->gsfTrack()->hitPattern().numberOfAllHits(reco::HitPattern::MISSING_INNER_HITS) );
      ele_isPassConVeto_.push_back ( el->passConversionVeto() );
      ele_isEcalDriven_.push_back ( el->ecalDrivenSeed() );
      ele_energy5x5_.push_back ( el->e5x5() );
      ele_energy2x5_.push_back ( el->e2x5Max() );
      ele_energy1x5_.push_back ( el->e1x5() );
      ele_eleRho_.push_back(rho_ );
      ele_eleTrkPt_.push_back ( el->dr03TkSumPt() );
      ele_GsfEleEmHadD1IsoRhoCut_.push_back ( el->dr03EcalRecHitSumEt() + el->dr03HcalDepth1TowerSumEt() );
      ele_isEE_.push_back (el->isEB() );    
      ele_isEB_.push_back (el->isEE() );   
...
//        printf("Electron energy is zero!\n");
      ...
      static bool passConvVeto = !ConversionTools::hasMatchedConversion(*el, *conversions, theBeamSpot->position()); 
